...
-   [3. MODELING RESULTS](#modeling_results)
...
# **3. MODELING RESULTS** {#modeling_results style="color: gray"}
...
```{r regularized_movie_effect_model, message=FALSE, warning=FALSE}
#########################################################
...
# **4. RESULTS** {#results style="color: gray"}
...
# **5. PCA (principal component analysis)** {#pca style="color: gray"}
...
# **6. CONCLUSION** {#conclusion style="color: #27408B"}
...
4.  Regularized Movie + User Effect Model: We use four variables: mu (median rating), b_i (movie bias), b_u (user bias), $\lambda$ (lambda). To the movie mean we add the deviations from the movie rating means and add the deviation of the average rating of the movies of the users and use lambda with the idea of ​​penalizing the outliers from the movie bias and user bias sets, which will optimize the recommendation system. **The result is: 0.86482 therefore objective completed, RMSE \< 0.86490.**
